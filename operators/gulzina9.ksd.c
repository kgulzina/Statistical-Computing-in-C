...
{			
	if (!strcmp(argv[1], "-f")) {
		...
		*adr3 = *adr1 + 1;
		...
		int8_t sh = (*adr1 >> 23 & 255) - 127;	/* [KSD] extract exponent */
		if (sh < 0) sh = -sh;
		printf("%e = %e\n", FLT_EPSILON, (new_value - f) / pow(2, sh)); 	//see the comments
	} else {
...
/* KSD: You do not need to zero out the exponent.  See my correction.
 * KSD: But, you do need to compute the relative difference, not the absolute difference to match FLT_EPSILON.
 */
